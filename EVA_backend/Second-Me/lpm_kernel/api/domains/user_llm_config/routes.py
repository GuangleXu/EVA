...
# OpenAI配置常量，优先从环境变量读取
OPENAI_ENDPOINT = os.environ.get("OPENAI_API_BASE_URL", "")
OPENAI_CHAT_MODEL = os.environ.get("OPENAI_MODEL", "")
...
def process_openai_config(data: Dict[Any, Any]) -> Dict[Any, Any]:
    """处理OpenAI配置，全部通过环境变量或配置文件注入，不做硬编码"""
    if data.get('provider_type') == 'openai' and data.get('key'):
        # 用统一key填充chat和embedding
        data['chat_api_key'] = data['key']
        data['chat_model_name'] = OPENAI_CHAT_MODEL or data.get('chat_model_name', '')
        data['embedding_api_key'] = data['key']
        data['embedding_model_name'] = OPENAI_EMBEDDING_MODEL or data.get('embedding_model_name', '')
        data['chat_endpoint'] = OPENAI_ENDPOINT or data.get('chat_endpoint', '')
        data['embedding_endpoint'] = OPENAI_ENDPOINT or data.get('embedding_endpoint', '')
    return data
