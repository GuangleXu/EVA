...
class L2Generator:
    ...
    def clean_graphrag_keys(self):
        """动态注入api_key、api_base等，禁止硬编码，优先用环境变量和配置文件"""
        import os
        import yaml
        # 优先从环境变量读取
        ...
        if (not api_key or not api_base or not model) and os.path.exists(config_path):
            try:
                with open(config_path, "r", encoding="utf-8") as f:
                    file_config = yaml.safe_load(f)
                    api_key = api_key or file_config.get("openai_api_key", "")
                    api_base = api_base or file_config.get("openai_api_base_url", "")
                    model = model or file_config.get("openai_model", "")
            except Exception as e:
        ...
        settings["models"]["default_chat_model"]["api_base"] = api_base
        settings["models"]["default_chat_model"]["model"] = model
